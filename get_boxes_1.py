...
#from visualize_case import visualize_file

""" Durchführung und Generieren der einzelnen Boxen """
...
def mask2D_to_bbox(gt2D, idx):
    """ vorsicht änderung """
    y_indices, x_indices = np.where(gt2D == idx)
...
def mask3D_to_bbox(gt3D, idx):
    b_dict = {}
    """ vorsicht änderung """
    z_indices, y_indices, x_indices = np.where(gt3D == idx)
    ...
    box_2d = mask2D_to_bbox(gt_mid, idx)
...
hauptordner = "/Users/ricardabuttmann/Desktop/CVPR/SegFM3D/3D_train_npz_random_10percent_16G/US"
ziel_ordner = "/Users/ricardabuttmann/Desktop/CVPR/SegFM3D/3D_train_boxes/US"
...
for ordner in unterordner:
    #print(ordner)
    ordner_pfad = os.path.join(hauptordner, ordner)

    dateien_liste = sorted([name for name in os.listdir(ordner_pfad) if name.endswith(".npz")])
    ...
    for datei in dateien_liste:
        ...
        data = np.load(datei_pfad, allow_pickle=True)
        ...
        indices = sorted(np.unique(gts))[1:]
        #print(max_idx)
        boxes_list = []

        for i in indices:
            dict = mask3D_to_bbox(gts, i)
            boxes_list.append(dict)

        boxes = np.array(boxes_list, dtype=object)

        speicher_pfad = os.path.join(ziel_ordner, ordner)
        os.makedirs(speicher_pfad, exist_ok=True)
        ziel_pfad = os.path.join(speicher_pfad, datei)

        np.savez(ziel_pfad, boxes = boxes)
